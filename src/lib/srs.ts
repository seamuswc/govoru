// ─── SRS Fluency engine — WaniKani timing ──────────────────────────────────
...
import { RU_CONJUGATIONS, RU_WORDS } from '@/data/ruDeck'
import { RU_VERB_FORMS, type VerbRow } from '@/data/ruVerbForms'
...
export interface Card {
  ...
  back: string
  kind: CardKind
  /** The lemma this card belongs to (e.g. "говорить"). Conjugations of the same
...
export function formatInterval(ms: number): string {
  ...
  const d = h / 24
  if (d < 30) return `${Math.round(d * 10) / 10}d`
  const mo = d / 30
...
export function gradePreview(card: Card): Record<Grade, string> {
  ...
  for (const g of ['again', 'hard', 'good', 'easy'] as Grade[]) {
    ...
    out[g] = next.level >= BURN_LEVEL ? 'Burned 🔥' : formatInterval(next.due)
  }
  return out
}
...
export function simplePreview(card: Card): { right: string; wrong: string } {
  ...
  return {
    right: r.level >= BURN_LEVEL ? 'Burned 🔥' : `next in ${formatInterval(r.due)}`,
    wrong: `back in ${formatInterval(w.due)}`,
  }
}
...
/** Parse pasted lines like "говорить — to speak" (also accepts - , ; or tab). */
...
const KEY_PREFIX = 'srs-fluency:v4:'
const ANON_KEY = 'srs-fluency:v4'
const ACCOUNTS_KEY = 'srs-fluency:accounts'
const CURRENT_KEY = 'srs-fluency:current'
// legacy keys to purge ('srs-fluency:v1' held the old Spanish demo deck)
const LEGACY_KEYS = ['srs-fluency:v1', 'srs-fluency:v2', 'srs-fluency:v3']
...
// ─── Seed deck: Русский, advanced-B2, ~10,700 cards out of the box ─────────
// 8,000 frequency-ordered word cards (OpenRussian.org dictionary data,
// CC BY-SA 4.0) + full conjugation paradigms for the top 300 verbs, where
// every single conjugated form is its own independent card.
...
function seedCards(): Card[] {
  ...
  // interleave one verb paradigm (9 forms) every 40 frequency words
  let ci = 0
  RU_WORDS.forEach((w, i) => {
    rows.push({ f: w.f, b: w.b, kind: 'word' })
    if ((i + 1) % 40 === 0 && ci < RU_CONJUGATIONS.length) {
      for (const c of RU_CONJUGATIONS.slice(ci, ci + 9)) {
        rows.push({ f: c.f, b: c.b, kind: 'conjugation', base: c.base })
      }
      ci += 9
    }
  })
  for (const c of RU_CONJUGATIONS.slice(ci)) {
    rows.push({ f: c.f, b: c.b, kind: 'conjugation', base: c.base })
  }
...
/** Look up a verb lemma (case-insensitive) in the 2,500-verb paradigm table. */
export function findVerb(lemma: string): VerbRow | null {
  if (!verbIndex) verbIndex = new Map(RU_VERB_FORMS.map((r) => [r[0].toLowerCase(), r]))
...
const VERB_FORM_TAGS = [
  'I · 1st sg. pres./fut.',
  'you · 2nd sg. pres./fut.',
  'he/she · 3rd sg. pres./fut.',
  'we · 1st pl. pres./fut.',
  'you · 2nd pl. pres./fut.',
  'they · 3rd pl. pres./fut.',
  'masc. past',
  'fem. past',
  'pl. past',
] as const

/** Turn a paradigm row into 9 conjugation card inputs (each its own card). */
